...
class TestFavoritesView(TestCase):
    ...
    def test_get_return_expected_page(self):
        assert 'favorites_app/favorites.html' in [template.name for template in self.response.templates]
        assert 200 == self.response.status_code

    def test_get_favorites_with_authentication_return_error(self):
